...
enum ChattingError: Error, @unchecked Sendable {
    case invalidConfig
...
class ChattingFeature: ObservableObject {
    ...
    private func sendWithStream(chat: Chat, receivingMessage: Message) async {
        ...
        do {
            guard let adapter = adapter(for: chat.model) else { return }
    ...
    private func send(message: Message) async {
        ...
        do {
            guard let chattingAdapter = adapter(for: chat.rawModel) else { return }
    ...
    private func adapter(for model: String?) -> ChattingAdapter? {
        if let model, let adapter = settingsFeature.modelToAdapter[model] {
            return adapter
        }

        return settingsFeature.chattingAdapters["hermes"]
    }
